...
class Parser:
    def parse(self, tokens: List[Tuple[Token, str]]) -> "ParseNode":
        self.tokens = tokens
        self.pos = 0
        self.end = len(tokens)
    ...
    def parse_function(self) -> "ParseNode":
        ...
        self.expects(Token.INT)
        name = self.expects(Token.IDENTIFIER)[1]
        self.expects(Token.LPAREN)
        self.expects(Token.VOID)
        self.expects(Token.RPAREN)
        self.expects(Token.LBRACE)
        statement = self.parse_statement()
        self.expects(Token.RBRACE)
        return Function(name, [statement])

    def parse_statement(self) -> "ParseNode":
        """Parse a return statement"""
        self.expects(Token.RETURN)
        expression = self.parse_expression()
        self.expects(Token.SEMICOLON)
        return Return([expression])

    def parse_expression(self) -> "ParseNode":
        """Parse a constant expression"""
        literal = self.expects(Token.CONSTANT)[1]
        return Constant(literal)

    def expects(self, expected_type: Token) -> Tuple[Token, str]:
...
class Program(ParseNode):
    ...
    def __init__(self, children: List["ParseNode"] = None):
